...
false && describe('retrieve pet visit', () => {
    ...
    it('should succeed on correct credentials', async () => {
        const visit = await logic.retrievePetVisit()
            
                // expect(user.id).toBe(userId)
                // expect(user.id).toBeEqual(_id)
                expect(visit.vaccionations).toBe(vaccionations)
                expect(visit.controls).toBe(controls)
                expect(visit.details).toBe(details)
                // expect(user.save).toBeUndefined()
        })
    })
}),   
